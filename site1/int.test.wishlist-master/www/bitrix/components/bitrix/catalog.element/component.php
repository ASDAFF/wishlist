...
if (isset($_REQUEST[$arParams["ACTION_VARIABLE"]]) && isset($_REQUEST[$arParams["PRODUCT_ID_VARIABLE"]]))
{
    ...
    if($action == "WISH" && $productID > 0){
        $APPLICATION->RestartBuffer();
                
        $newElement = new CIBlockElement;
        $prop = array(
            "user" => $USER->GetId(),
            "element_title" => $_REQUEST["element_title"],
            "element_href" => $_REQUEST["element_href"],
            "element_pic" => $_REQUEST["element_pic"],
        );
        ...
        //$newElement->Add($arLoadElement)
        if($PRODUCT_ID = $newElement->Add($arLoadElementArray)){
            echo CUtil::PhpToJSObject("New ID: ".$PRODUCT_ID);            
        }
        else{            
            echo CUtil::PhpToJSObject("Error: ".$newElement->LAST_ERROR);
        }
        die();        
    }
}
